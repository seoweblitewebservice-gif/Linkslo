          ...
          <article key={gig.id} className="card-hover group flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-white shadow-soft">
            <Link href={`/marketplace/gigs/${gig.slug}`} className={`relative block h-32 overflow-hidden bg-gradient-to-br ${COVERS[index % COVERS.length]} p-5 text-white`}>
              ...
              <div className="relative flex h-full flex-col justify-between">
                <div className="flex items-center justify-between"><span className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/20 bg-white/10"><Icon name="link" size={18} /></span><Badge tone="brand">Featured listing</Badge></div>
                <p className="text-[0.66rem] font-semibold uppercase tracking-[0.12em] text-white/75">{gig.category}</p>
              </div>
            </Link>
            <div className="flex flex-1 flex-col p-5">
              <div className="flex items-center gap-2.5">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-ink-950 text-white"><Icon name={gig.domain ? "globe" : "link"} size={14} /></span>
                <span className="min-w-0 flex-1"><span className="block truncate text-[0.76rem] font-semibold text-ink-900">{gig.domain ?? gig.industry}</span><span className="block truncate text-[0.65rem] text-ink-400">{gig.country} · {gig.language}</span></span>
              </div>
              ...
              <p className="mt-2 line-clamp-2 text-[0.76rem] leading-relaxed text-ink-500">{gig.summary}</p>
