        ...
        return <section key={group}>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((tool) => <Link key={tool.slug} href={`/${tool.slug}/`} className="group flex h-full flex-col rounded-2xl border border-line bg-white p-5 transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-soft">
              <div className="flex items-start justify-between gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-50 text-brand-700"><Icon name={tool.category === "Structured Data" ? "code" : tool.category === "XML Sitemaps" ? "document" : tool.category === "Social & SERP" ? "eye" : "sliders"} size={17} /></span><span className="rounded-full bg-canvas px-2.5 py-1 text-[0.63rem] font-semibold uppercase tracking-wide text-ink-400">Free</span></div>
